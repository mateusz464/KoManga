...
**Status:** Blocked (needs API-905/906)
...
**Estimate:** M
**Blocker (2026-06-29):** This cannot be done as a plugin-only change. The API exposes only a raw Suwayomi `thumbnailUrl` (search + `GET /api/manga/:id`) and has **no profile-negotiated cover-image endpoint** — the sole image endpoint, `GET /api/page/:id?profile=eink`, serves chapter pages, not covers. The client must not reach Suwayomi or the raw URL directly (RFC §6/§13, CLAUDE.md §6/§10), and the `eink` profile this ticket requires is produced server-side. Raised **API-905** ([TEST]) / **API-906** (impl) in the API epic's 9xx block to add `GET /api/manga/:id/cover?profile=eink`. Once shipped, the plugin work is: an `ApiClient:coverImageUrl(mangaId)` builder (always `profile=eink`, mirroring `pageImageUrl`/`cbzUrl`), fetched non-blocking + wifi-gated via `net.lua`, rendered on a cover-capable menu with a missing/failed cover degrading to text.
