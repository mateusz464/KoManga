...
### API-808 — Make the Cloudflare Tunnel optional (local-only Compose profile) — **Done**
**Description:** The root `docker-compose.yml` hard-required `CLOUDFLARE_TUNNEL_TOKEN` (the `cloudflared` service used `${CLOUDFLARE_TUNNEL_TOKEN:?…}`), so `docker compose up` failed fast when the token was unset — forcing every user to set up a Cloudflare Tunnel even for LAN-only reading. Make the tunnel opt-in.
**Acceptance criteria:**
- `docker compose up -d` with a blank/absent `CLOUDFLARE_TUNNEL_TOKEN` starts `suwayomi` + `api` cleanly (no fail-fast, no `cloudflared`).
- `docker compose --profile tunnel up -d` starts the connector as before; the token guard still fails fast when the profile is active but the token is unset.
- Suwayomi remains internal-only in both modes (RFC §9); no change to its port exposure.
- `.env.example` updated to note the token is only required for the tunnel profile.
- `README.md` and `docs/cloudflare-tunnel.md` updated: local-only vs. tunnel run commands.
**Blocked by:** none. **Estimate:** S
**Notes (2026-07-06):** Put `cloudflared` behind a Compose `profiles: ["tunnel"]`, so a plain `docker compose up -d` never creates it (LAN-only, no token). **Reconciliation:** the ticket assumed profiles would stop the `${…:?}` guard from being evaluated, but Compose (verified on the local `v5.2.0`) interpolates the whole file *before* applying profiles, so a `:?` on `cloudflared` still failed a tokenless local-only `up`. Defaulted `TUNNEL_TOKEN: ${CLOUDFLARE_TUNNEL_TOKEN:-}` to remove the global mandatory-variable error, and preserved the fail-fast with a tiny profile-gated `tunnel-preflight` (busybox) service that exits non-zero with a clear message when the token is unset; `cloudflared` gains `depends_on: tunnel-preflight (service_completed_successfully)`, so under `--profile tunnel` a missing token blocks the connector and fails the `up`. Verified via `docker compose config`/`up --dry-run`: base = `api`+`suwayomi` only (no fail-fast); `--profile tunnel` adds `cloudflared`+`tunnel-preflight`; preflight run exits 1 (blank) / 0 (set); Suwayomi publishes no host port in either mode. Updated `.env.example`, `README.md`, `docs/cloudflare-tunnel.md`, and the compose header/service comments.
